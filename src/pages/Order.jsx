  ...
  return (
    <div>
     ...
     <div>
      {
        orders.map((order)=>(
          <div className='grid grid-cols-1 sm:grid-cols-[0.5fr_2fr_1fr] lg:grid-cols-[0.5fr_2fr_1fr_1fr_1fr] gap-3 items-start border-gray-200 p-5 md:p-8 my-5 md:my-4 text-xs sm:text-sm text-gray-700 ' key={order._id}>
            ...
            <p>{order.address.province + ", "+order.address.district+", "+order.address.zipcode}</p>
            </div>
          ...
          <p>Date : {new Date(order.date).toLocaleDateString()}</p></div>
